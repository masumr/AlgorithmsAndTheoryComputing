 ...
 class Convergence{
  ...
  public float calculateSum(int n) { //this will calculate Sum of the series, if the series changes, this function will change.
    for(int i = 0; i < n; i++, denominator++) {
        sum = sum + (float)(Math.pow( -1, denominator+1 ) * (1/((2 * denominator) - 1)));
        }
      return sum;
    }
    public void isConvergent(float[] array){
      //this will finally calculate if the sum of the series is convergent or not!
        ...
        if (flag){
            System.out.println("The Given Series is Likely Convergent.");
            System.out.println("Printing one of the sum value (x 4 times) : " + (4*array[9]));
            System.out.println("\n");
        }
        else{
...
class MyInfo{
  public void identity(){
    ...
    try {
        assingmentDate = dateformat3.parse("09/SEP/2016"); //Date printing, using the DATE class in java.
      } 
      catch (ParseException ex) {
          System.out.print(ex);
      }
    System.out.println("Assingment(1B) done on : "+dateformat3.format(assingmentDate));
    System.out.println("---------------------------");
  }
}